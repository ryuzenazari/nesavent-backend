...
mongoose.connect(process.env.MONGODB_URI)
  ...
  .catch(err => {
    logger.error('Gagal terhubung ke MongoDB', { error: err.message });
    process.exit(1);
  });
async function runSeed() {
  try {
    ...
    const event1 = await createEvent(
      ...
      [
        ...
        },
        {
          ...
          price: 150000,
          quantity: 50,
          benefits: ['Akses ke area festival', 'Merchandise festival', 'Meet & Greet dengan band', 'Akses area VIP']
        },
        {
          name: 'Student',
    ...
    const event3 = await createEvent(
      ...
      [
        ...
        },
        {
          ...
          price: 250000,
          quantity: 10,
          benefits: ['Akses ke workshop untuk 3 orang', 'Material workshop', 'Sertifikat', 'Konsultasi khusus']
        },
        {
          name: 'Student',
          ...
          price: 50000,
          quantity: 20,
          benefits: ['Akses ke workshop', 'Material workshop', 'Sertifikat', 'ID mahasiswa harus ditunjukkan']
        }
      ]
    );
    ...
    logger.error('Error saat seeding database', { error: error.message, stack: error.stack });
    process.exit(1);
  }
}
...
async function createEvent(title, description, date, time, location, category, organizer, creatorId, ticketTypesData) {
  ...
  const event = new Event({
    ...
    price: {
      regular: ticketTypesData.find(t => t.name === 'Regular' || t.name === 'Basic' || t.name === 'Individual')?.price || 0,
...
async function createTicket(event, user, ticketType) {
  try {
    ...
    await Event.updateOne(
      ...
      { $inc: { 'ticketTypes.$.availableQuantity': -1, 'availableTickets': -1 } }
    );
