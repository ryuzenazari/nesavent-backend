...
const verifyEventCreator = (req, res, next) => {
  ...
  if (req.user.role !== 'creator' && req.user.role !== 'staff_creator' && req.user.role !== 'admin') {
    logger.warn(`Event creator access denied for user ${req.user.email} with role ${req.user.role}`);
    return res.status(403).json({ 
      success: false,
...
const verifyTicketChecker = (req, res, next) => {
  ...
  if (req.user.role !== 'staff_creator' && req.user.role !== 'creator' && req.user.role !== 'admin') {
