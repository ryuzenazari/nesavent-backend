...
app.get('/', (req, res) => {
  res.json({
    ...
    disclaimer: 'NesaVent adalah platform independen dan tidak terafiliasi secara resmi dengan Universitas Negeri Surabaya (UNESA)'
  });
});
...
app.use((error, req, res, next) => {
  logger.error('Kesalahan server', { error: error.message, stack: error.stack });
...
if (require.main === module) {
  connectDB()
    ...
    .catch(err => {
      logger.error('Error saat koneksi/memulai server', { error: err.message });
      process.exit(1);
    });
}
