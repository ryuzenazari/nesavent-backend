...
exports.validateTicket = async (req, res) => {
  try {
    ...
    const ticket = await Ticket.findById(qrData.tid)
      .populate('event')
      .populate('user');
    if (!ticket) {
      logger.warn('Percobaan validasi tiket yang tidak ada', { qrData });
    ...
    logger.error('Error validasi tiket', { error: error.message });
...
exports.getEventCheckInStats = async (req, res) => {
  try {
    ...
    const percentCheckedIn = totalTickets > 0 
    ...
    const formattedCheckIns = recentCheckIns.map(ticket => ({
      ...
      scannedBy: ticket.checkInInfo?.scannedBy ? {
        name: ticket.checkInInfo.scannedBy.staffDetails?.staffName || ticket.checkInInfo.scannedBy.name,
    ...
    logger.error('Error mendapatkan statistik check-in', { error: error.message });
    return res.status(500).json({
      success: false,
