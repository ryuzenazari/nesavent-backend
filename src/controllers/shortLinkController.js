...
const getTargetUrl = (shortLink) => {
  ...
  if (shortLink.targetType === 'event' && shortLink.targetId) {
    return `${baseUrl}/events/${shortLink.targetId._id || shortLink.targetId}`;
  }
  
  if (shortLink.targetType === 'ticket' && shortLink.targetId) {
    return `${baseUrl}/tickets/${shortLink.targetId._id || shortLink.targetId}`;
  }
...
const createNewShortLink = async (req, res) => {
  try {
    ...
    if ((targetType === 'event' || targetType === 'ticket') && !mongoose.Types.ObjectId.isValid(targetId)) {
...
const getUserShortLinks = async (req, res) => {
  try {
    ...
    const shortLinks = await ShortLink.find({ createdBy: req.userId })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .populate('targetId', 'title name');
    
    const totalLinks = await ShortLink.countDocuments({ createdBy: req.userId });
    
    const formattedLinks = shortLinks.map(link => {
      const shortUrl = `${process.env.API_URL || req.protocol + '://' + req.get('host')}/s/${link.code}`;
      return {
        code: link.code,
        shortUrl,
    ...
    res.status(500).json({ message: 'Terjadi kesalahan saat mengambil daftar shortlink', error: error.message });
  }
};
const updateShortLink = async (req, res) => {
  try {
    ...
    if (!updatedShortLink) {
      return res.status(404).json({ message: 'Shortlink tidak ditemukan atau Anda tidak memiliki izin' });
    }
    
    const shortUrl = `${process.env.API_URL || req.protocol + '://' + req.get('host')}/s/${updatedShortLink.code}`;
    ...
    res.status(500).json({ message: 'Terjadi kesalahan saat mengupdate shortlink', error: error.message });
  }
};
const deleteShortLink = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Shortlink berhasil dihapus' });
    
  } catch (error) {
    logger.error(`Error saat menghapus shortlink: ${error.message}`);
    res.status(500).json({ message: 'Terjadi kesalahan saat menghapus shortlink', error: error.message });
  }
};
  ...
  try {
    ...
    const formatLink = (link) => {
      ...
      return {
        ...
        shortUrl,
        targetType: link.targetType,
        targetName: link.targetId ? (link.targetId.title || link.targetId.name) : (link.customUrl || 'External Link'),
        visits: link.visits,
